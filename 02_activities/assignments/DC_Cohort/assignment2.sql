...
/* 1. using ROW_NUMBER() to number each visit including purchase details */
SELECT 
	customer_id,
	market_date,
...
/* 2. using DENSE_RANK() to number unique market dates per customer */
SELECT 
	customer_id,
	market_date,
...
WITH sales_per_date AS (
	...
	SELECT 
		market_date,
		total_sales,
		DENSE_RANK() OVER (ORDER BY total_sales DESC) AS sales_rank_desc,
		DENSE_RANK() OVER (ORDER BY total_sales ASC) AS sales_rank_asc
	FROM 
		sales_per_date
)
...
WITH vendor_products AS (
    SELECT 
        ...
        vi.original_price
    FROM vendor v
    JOIN vendor_inventory vi USING (vendor_id)
...
VALUES (9999, 'Sample Product', '1 unit', 1, 'unit', CURRENT_TIMESTAMP);
